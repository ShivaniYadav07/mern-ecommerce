...
export const updateProfile = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(`${SERVER_ENDPOINT}/api/v1/me/update`, userData, config);
...
export const forgotPassword = (email) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`${SERVER_ENDPOINT}/api/v1/password/forgot`, email, config);
...
export const resetPassword = (token, passwords) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(`${SERVER_ENDPOINT}/api/v1/password/reset/${token}`, passwords, config);
...
export const getUserDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${SERVER_ENDPOINT}/api/v1/admin/user/${id}`);
